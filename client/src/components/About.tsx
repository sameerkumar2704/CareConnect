...
const About = () => {
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#A9E2E3] to-[#00979D] px-6 py-12">
            ...
            <div className="mt-8 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {teamMembers.map((member, index) => (
                    <div key={index} className="bg-white shadow-lg rounded-lg p-6 text-center border-t-4 border-[#00979D]">
                        ...
                        />
                        <h2 className="text-xl font-semibold text-gray-800 mt-4">{member.name}</h2>
                    </div>
                ))}
            </div>
        </div>
